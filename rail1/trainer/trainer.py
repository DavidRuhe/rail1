...
from rail1.callbacks.checkpoint import Checkpoint
from rail1.loggers.loggers import ConsoleLogger, WANDBLogger


def print_git_state():
    branch = run_bash_command("git rev-parse --abbrev-ref HEAD")
    commit = run_bash_command("git rev-parse HEAD")
    message = run_bash_command("git log -1 --pretty=%B")

    # Get the tag associated with that commit, if it exists
    tag = subprocess.getoutput(f"git tag --contains {commit}")
...
class Trainer:
    def __init__(
        ...
        logger: Any = None,
        max_steps: int = 0,
        max_time: str = None,  # type: ignore
        limit_val_batches: int = float("inf"),  # type: ignore
        ...
        if limit_val_batches < float("inf"):
            print(
                f"Limiting the number of validation and test batches to {limit_val_batches}."
            )
    ...
    def train_step(
        ...
        loss, outputs = model(batch, self.global_step, prefix)
        ...
        if self.is_distributed:
            ...
            model.train_metrics.update(**outputs)  # type: ignore
    ...
    def test_loop(
        ...
        if self.is_distributed:
            ...
            assert model.test_metrics.empty()  # type: ignore
        if validation:
            ...
            for batch in train_loader:
                ...
                if self.global_step % self.log_interval == 0:
                    ...
                    if self.is_distributed:
                        ...
                        train_metrics = model.train_metrics.compute()  # type: ignore
                        model.train_metrics.reset()  # type: ignore
                ...
                should_validate = self.global_step % self.val_check_interval == 0 and (
                    self.global_step > 0 if self.skip_initial_eval else True
                )

                if should_validate:
                    if val_loader is not None and self.limit_val_batches > 0:
                        if self.global_step == 0 and self.skip_initial_eval:
                    ...
                    if self.should_test:
                        if test_loader is not None:
                            with torch.no_grad():
                                ...
                                self.should_test = False
                        torch.save(model.state_dict(), "csmp/exp_results/motion/model1.pth")
