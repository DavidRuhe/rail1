...
def knn_and_center(loc, new_loc, features, new_features, k, concat_xyz=True, concat_new_features=True):
    ...
    if features is not None:
        ...
        if concat_new_features:
            knn_features = torch.cat([knn_features, new_features[:, :, None].expand(-1, -1, k, -1)], dim=-1)
                # if concat_xyz:
...
class PointTransformerCls(nn.Module):
    def __init__(self):
        ...
        self.l1 = conv1d_batchnorm_act(d_points, 64)
        self.l2 = conv1d_batchnorm_act(64, 64)
