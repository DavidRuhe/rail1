...
class ShapesFNF(nn.Module):
    ...
    def __init__(self, input_dim, output_dim, input_conditioning_dim):
        ...
        self.mlp2 = nn.Sequential(nn.Linear(2, 128), nn.ReLU(), nn.Linear(128, 32))
...
class RFF(nn.Module):
    def __init__(self, input_dim, hidden_dim, learnable_coefficients=True, std=1.0):
        ...
        self.coefficients.weight.data = torch.arange(1, hidden_dim // 2 + 1).float()[:, None]
        # Store pi
        self.pi = math.pi
...
class FNF(nn.Module):
    ...
    def forward(self, input):
        basis = torch.eye(3, device=input.device)
        ip = torch.einsum('...j, ij->...i', input, basis)
        norm = input.norm(dim=-1, keepdim=True)
        x = torch.cat([ip, norm], dim=-1)
        ff = self.ff(x[..., None])
        ff = ff.reshape(*input.shape[:-1], -1)
        ff = torch.cat([ff, ip, norm], dim=-1)
        # ff = input
        return self.mlp(ff)
