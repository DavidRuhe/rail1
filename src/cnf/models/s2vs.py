...
class AutoEncoder(nn.Module):
    ...
    def encode(self, pc):
        # pc: B x N x 3
        B, N, D = pc.shape
        assert N == self.num_inputs, f'Input size mismatch: {N} != {self.num_inputs}'
...
def create_autoencoder(dim=512, M=512, latent_dim=64, N=1024, determinisitc=False):
    if determinisitc:
        model = AutoEncoder(
            depth=24,
            dim=dim,
