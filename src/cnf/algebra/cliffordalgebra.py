...
class CliffordAlgebra(nn.Module):
    ...
    def random_grade(self, grade: int, n=None):
        if n is None:
            n = 1
        grade_indices = self.bbo_grades == grade
        mv = torch.zeros(n, self.n_blades, device=self.cayley.device)
        mv[:, grade_indices] = torch.randn(
            n, grade_indices.sum(), device=self.cayley.device
        )
        return mv

    def random_vector(self, n=None):
        return self.random_grade(1, n=n)
