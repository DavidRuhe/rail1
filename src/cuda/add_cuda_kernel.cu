...
template <typename scalar_t>
__global__ void add_cuda_forward_kernel(
    ...
    torch::PackedTensorAccessor32<scalar_t, 1, torch::RestrictPtrTraits> new_h
...
)
{
    ...
    if (c < input_left.size(0))
    {
        ...
        new_h[c] = input_left[c] + input_right[c];
    }
}

template <typename scalar_t>
__global__ void add_cuda_backward_kernel(
...
)
{
    ...
    if (c < d_left.size(0))
    {
        ...
        d_left[c] = grad_result[c];
        d_right[c] = grad_result[c];
    }
}
...
)
{
    ...
    auto new_h = torch::zeros_like(input_left);
    ...
    AT_DISPATCH_FLOATING_TYPES(new_h.type(), "add_forward_cuda", ([&]
                                                                  { add_cuda_forward_kernel<scalar_t><<<blocks, threads>>>(
                                                                        ...
                                                                        new_h.packed_accessor32<scalar_t, 1, torch::RestrictPtrTraits>()
...
)
{
    ...
    AT_DISPATCH_FLOATING_TYPES(grad_h.type(), "add_backward_cuda", ([&]
